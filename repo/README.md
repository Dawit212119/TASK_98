...
| `merchant` | **403** — reservations have no merchant-assignment column; this endpoint is not used for merchant views. |
| `analytics_viewer` | **403** unless the user also has `patient`, `staff`, `provider`, or `ops_admin` (this route is not the analytics reporting API). |
...
| **2) Reservation list RBAC** | Row scope by role; merchant / analytics-only forbidden; provider scope. | `reservation-list-scope.spec.ts` (query shape + 403 for forbidden roles; `analytics_viewer`+`patient` allows patient scope). | Patient list self-only; cross-patient exclusion; merchant & analytics_viewer list **403** `RESERVATION_LIST_FORBIDDEN`; staff clinic list; **ops_admin** list; **provider** list includes rows where `provider_id` is the caller. |
...
| **7) Content-quality analytics** | `share` event ingestion and completion/engagement/share aggregation output. | — | `analytics/events` ingestion + `analytics/aggregations/content-quality` assertions in `API_tests/run_api_tests.sh`. |
